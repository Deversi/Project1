            ...
            if(plugin_args && sections && sections_size && (type == EMU_PS3 || type == EMU_DVD || type == EMU_BD || type == EMU_PSX))
            {
                ...
                if(!strcmpext(path, ".iso.0"))
                {
                    ...
                    for (o = 1; o < 64; o++)
                    {
                        ...
                        if(stat(TEMP_PATH1, &s) != SUCCESS) break;
             ...
             || type == EMU_DVD || type == EMU_BD)
        {
            if(is_ps2_game)
            {
                ...
                if(type == EMU_DVD)
                    ...
                    ret = cobra_mount_ps2_disc_image(files, nfiles, (TrackDef *) TEMP_PATH1, 0);
                else ret = FAILED;
    ...
    while(true)
    {
        ...
        if(set_menu2)
        {
            ...
            if(max_menu2 == 6)
            {
                ...
                display_ttf_string(0, py, "Exit File Manager", (set_menu2 == 6  && (frame & BLINK)) ? INVISIBLE : WHITE, 0, 16, 24); py += 24;
            }
            else
            {
