/*
    ...
    HouyaManager (HMANAGER port) (c) 2011 D_Skywalk <http://david.dantoine.org>
...
int get_icon(char * path, const int num_dir)
{
    ...
    if (!is_update)
    {
        if(strlen(directories[num_dir].title_id) == 9 && strstr("NETBROWSE|HouyaMAN00|PRXLOADER|CDGPLAYER|GMPADTEST|BLES80616", directories[num_dir].title_id) != NULL)
        {
            if(strncmp(directories[num_dir].title_id, "HouyaMAN00", 9) == SUCCESS)
...
void fun_exit()
{
    ...
    if(file_exists("/dev_hdd0/game/HouyaMAN00/USRDIR/SHOWTIME.SELF"))
    {
        mkdir("/dev_hdd0/game/HouyaMAN00/USRDIR/TEMP", S_IRWXO | S_IRWXU | S_IRWXG | S_IFDIR);
        sysLv2FsChmod("/dev_hdd0/game/HouyaMAN00/USRDIR/TEMP", FS_S_IFMT | 0777);

        sysLv2FsUnlink("/dev_hdd0/game/HouyaMAN00/USRDIR/TEMP/SHOWTIME.TXT");

        sysLv2FsLink(video_path, "/dev_hdd0/game/HouyaMAN00/USRDIR/TEMP/SHOWTIME.TXT");
        sysLv2FsChmod("/dev_hdd0/game/HouyaMAN00/USRDIR/TEMP/SHOWTIME.TXT", FS_S_IFMT | 0777);

        strcpy(self_path, "/dev_hdd0/game/HouyaMAN00");
    ...
    if(file_exists("/dev_hdd0/game/HouyaMAN01/USRDIR/prxloader.self"))
    {
        sysProcessExitSpawn2("/dev_hdd0/game/HouyaMAN01/USRDIR/prxloader.self", NULL, NULL, NULL, 0, 3071, SYS_PROCESS_SPAWN_STACK_SIZE_1M);
        exit(0);
    }

    sysProcessExitSpawn2("/dev_hdd0/game/HouyaMAN00/USRDIR/prxloader.self", NULL, NULL, NULL, 0, 3071, SYS_PROCESS_SPAWN_STACK_SIZE_1M);
#endif
#else
...
void read_settings()
{
    ...
    strcpy(custom_homebrews, "HTSS00003|NETBROWSE|CDGPLAYER|GMPADTEST|HouyaMAN00|BLES80616|IMANAGER4|GMANAGER9|PRXLOADER|RBGTLBOX2|BLES80608|BLES80688|SSNE10000|SNES90000|VBAM90000|GENP00001|FCEU90000|FBAN00000|FBAL00123|MAME90000|PSID81257|PS2L00123|ROGE12345|CNDR00001|BLJS10018|");
    ...
    if(file)
    {
        ...
        if(bSpoofVersion)
        {
            ...
            if(spoof_version < 800) spoof_version = (((u32)(spoof_version/100)) * 0x100) + (((u32)((spoof_version % 100)/10)) * 0x10) + (spoof_version % 10); // convert to hex: 465 -> 0x465
        }
...
s32 main(s32 argc, const char* argv[])
{
    ...
    if (strstr(self_path, "/BLES80608")) sprintf(self_path, "/dev_hdd0/game/HouyaMAN00");
    ...
    if(use_mamba && !use_cobra)
    {
        ...
        // relaunch Houya manager to get vsh process for mamba
    ...
    if(mem)
    {
        ...
        if(!(strcasestr(directories[currentgamedir].path_name, ".iso") && file_exists("/dev_hdd0/game/HouyaMAN00/sprx_iso") == false))
            strcpy(self_path, "/dev_hdd0/game/HouyaMAN00");
...
autolaunch_proc:
                ...
                else
                {
                    ...
                    if(use_cache)
                    {
                        ...
                        {
                            // repair bug in some Houya Manager versions
...
void draw_options(float x, float y, int index)
{
    ...
    if(strlen(directories[currentgamedir].title_id) == 9 && strstr("HTSS00003|NETBROWSE|HouyaMAN00", directories[currentgamedir].title_id) != NULL)
    {
...
void draw_toolsoptions(float x, float y)
{
       ...
       (!(select_option == 4 || select_option == 5) && (new_pad & BUTTON_RIGHT)) )
    {
        switch(select_option)
        {
            ...
            case 3: // Copy from /dev_usb/Houya to Houya
                if(test_ftp_working()) break;
                copy_usb_to_Houya(self_path);
                break;
