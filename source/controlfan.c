...
bool test_controlfan_compatibility()
{
    if(firmware == 0x341C || firmware == 0x355C || firmware == 0x355D || firmware == 0x421C || firmware == 0x421D ||
	   ...
	   firmware == 0x465C || firmware == 0x465D
      )
        return true;
    else
        return false;
...
int load_ps3_controlfan_payload()
{
    ...
    if(firmware == 0x341C) {        // firmware 3.41 2EB128
        ...
        ret = 1;
    }  else if(firmware == 0x465C) { // firmware 4.65
...
void load_controlfan_config()
{
    ...
    if(file_exists(temp_buffer + 2048) == false)
        sprintf(temp_buffer + 2048, "/dev_hdd0/game/HOUYAMAN00/config/fancontrol.dat");
